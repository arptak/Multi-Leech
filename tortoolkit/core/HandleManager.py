...
async def handle_leech_command(e):
    if not e.is_reply:
        ...
        if await get_config() is not None:
            buts.append(
                [KeyboardButtonCallback("Upload To Drive 🌍", data=f"leechselect drive {tsp}")]
            )
...
async def start_handler(event):
    await event.reply(
        f"**TorToolKit-X Running Successfully!**\n__System Uptime:__ `{str(datetime.now() - START_TIME).split('.')[0]}`",
        link_preview=False,
        )
...
async def about_me(message):
    ...
    msg = (
        f"<b>Name:</b> <a href='https://t.me/TGFilmZone'>TGFilmZone</a>\n"
...
async def booted(client):
    ...
    for i in chats:
        try:
            await client.send_message(i, "BOT RESTARTED ✅.")
        except Exception:
